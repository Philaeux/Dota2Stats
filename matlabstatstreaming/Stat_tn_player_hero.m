function Stat_tn_player_hero(conn,hero,player_match_valve,match_valve,tn_id)
...
switch nargin
    case 4
        ...
        for k=1:nb_tn
            ...
            for m=1:nb_player
                ...
                for i=1:height(hero)
                    CalcStatHero=player_match_valve3(player_match_valve3.hero_id==hero.hero_id(i,1),:);
                    if height(CalcStatHero)>0
                        StatHerosadd=table();
                        ...
                        StatHerosPlayer=[StatHerosPlayer;StatHerosadd];
                    end
                end
                pgsqlexec(conn,['delete from grenouilleapi.public.stat_tn_player_hero where account_id=',num2str(list_player(m)),' and id_tn=',num2str(list_tn(k))])
                insert(conn,'grenouilleapi.public.stat_tn_player_hero',{'hero_id','id_tn','account_id','nb_pick','nb_ban','nb_match','mean_is_win','max_is_win','min_is_win','tot_is_win','mean_kills','max_kills','min_kills',...
                    ...
                    'tot_tower_damage','mean_level','max_level','min_level','tot_level','mean_hero_healing','max_hero_healing','min_hero_healing','tot_hero_healing'},StatHerosPlayer);
    ...
    case 5
        ...
        for m=1:nb_player
            ...
            pgsqlexec(conn,['delete from grenouilleapi.public.stat_tn_player_hero where account_id=',num2str(list_player(m)),' and id_tn=',num2str(tn_id)])
            insert(conn,'grenouilleapi.public.stat_tn_player_hero',{'hero_id','id_tn','account_id','nb_pick','nb_ban','nb_match','mean_is_win','max_is_win','min_is_win','tot_is_win','mean_kills','max_kills','min_kills',...
                'tot_kills','mean_assists','max_assists','min_assists','tot_assists','mean_deaths','max_deaths','min_deaths','tot_deaths','mean_last_hits','max_last_hits','min_last_hits',...
                'tot_last_hits','mean_denies','max_denies','min_denies','tot_denies','mean_gold_per_min','max_gold_per_min','min_gold_per_min','tot_gold_per_min','mean_xp_per_min',...
                'max_xp_per_min','min_xp_per_min','tot_xp_per_min','mean_hero_damage','max_hero_damage','min_hero_damage','tot_hero_damage','mean_tower_damage','max_tower_damage','min_tower_damage',...
