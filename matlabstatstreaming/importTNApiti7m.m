function [Tournoi,Team,Player]=importTNApiti7m(TN_Name,TN_api_url,TN_api_key,TN_api_token)
...
for i=1:height(Team)
    AddPlayer=table();
    Team.icon_small{i,1}=dataTN.logo(i,1).icon_small;
    Team.icon_medium{i,1}=dataTN.logo(i,1).icon_medium;
    Team.icon_large_square{i,1}=dataTN.logo(i,1).icon_large_square;
    Team.extra_small_square{i,1}=dataTN.logo(i,1).extra_small_square;
    Team.medium_small_square{i,1}=dataTN.logo(i,1).medium_small_square;
    Team.original{i,1}=dataTN.logo(i,1).original;
    ...
    AddPlayer.country=dataTeam.country;
    for j=1:nbplayer
        AddPlayer.team_id(j,1)=Team.team_id(i,1);
        AddPlayer.team_name(j,1)=Team.name(i,1);
        ...
        AddPlayer.ti_winner(j,1)=str2double(datateam.lineup(j).custom_fields_private(7).value);
    end
