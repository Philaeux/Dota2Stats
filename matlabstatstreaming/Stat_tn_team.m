...
switch type
    ...
    case 'qualif'
        ...
        if nb_team>0
            %% Bounty prise / bounty disponible init
            rq_bounty=['select account_id,player_slot,matchs.is_qualif,openmatch.id_tn,openmatch.radiant_team_id,openmatch.dire_team_id,hero_id,openmatch.match_id,bounty_pickups from openplayermatch inner join openmatch on openplayermatch.match_id = openmatch.match_id inner join matchs on openplayermatch.match_id = matchs.match_id where openmatch.id_tn =',num2str(tn_id),' and matchs.is_qualif = 1'];
            match_player_bounty=pgsqldata(conn,rq_bounty);
    ...
    case 'all'
        ...
        if nb_team>0
            %% Bounty prise / bounty disponible init
            rq_bounty=['select account_id,player_slot,matchs.is_qualif,openmatch.id_tn,openmatch.radiant_team_id,openmatch.dire_team_id,hero_id,openmatch.match_id,bounty_pickups from openplayermatch inner join openmatch on openplayermatch.match_id = openmatch.match_id inner join matchs on openplayermatch.match_id = matchs.match_id where openmatch.id_tn =',num2str(tn_id)];
            match_player_bounty=pgsqldata(conn,rq_bounty);
