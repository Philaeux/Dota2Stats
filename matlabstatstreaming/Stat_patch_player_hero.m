function Stat_patch_player_hero(conn)
disp('Traitement des stats patch Player Heros')
...
if nb_match>0
    ...
    rqexist=['select id from public.stat_patch_player_hero where patch_num=',num2str(patchnum)];
    exist1=pgsqldata(conn,rqexist);
    if strcmp(exist1,'No Data')==0
        pgsqlexec(conn,['delete from public.stat_patch_player_hero where patch_num=',num2str(patchnum)])
    end
    CBM_PGSQL_Transact_light(conn,'stat_patch_player_hero',StatHerosPlayer.Properties.VariableNames,StatHerosPlayer,'id','public');
end
disp('Traitement OK')
end