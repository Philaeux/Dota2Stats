function Stat_global_team(conn)
disp('Traitement des stats Global Team')
match_valve_tn=pgsqldata(conn,'select * from public.join_valvematch');
...
rq_bounty='select account_id,player_slot,is_qualif,id_tn,radiant_team_id,dire_team_id,hero_id,match_id,bounty_pickups from join_openplayermatch';
...
if nb_team>0
    ...
    rqexist='select id from public.stat_global_team';
    exist1=pgsqldata(conn,rqexist);
    if strcmp(exist1,'No Data')==0
        pgsqlexec(conn,'delete from public.stat_global_team')
    end
    CBM_PGSQL_Transact_light(conn,'stat_global_team',StatTeam.Properties.VariableNames,StatTeam,'id','public');
end
