function Stat_global_player_hero(conn)
disp('Traitement des stats Global Player Heros')
...
if nb_match>0
    ...
    rqexist='select id from public.stat_global_player_hero';
    exist1=pgsqldata(conn,rqexist);
    if strcmp(exist1,'No Data')==0
        pgsqlexec(conn,'delete from public.stat_global_player_hero')
    end
    CBM_PGSQL_Transact_light(conn,'stat_global_player_hero',StatHerosPlayer.Properties.VariableNames,StatHerosPlayer,'id','public');
end
disp('Traitement OK')
end