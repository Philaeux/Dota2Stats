...
if nb_match>0
    ...
    rqexist='select id from public.stat_global_team_hero';
    exist1=pgsqldata(conn,rqexist);
    if strcmp(exist1,'No Data')==0
        pgsqlexec(conn,'delete from public.stat_global_team_hero')
    end
    CBM_PGSQL_Transact_light(conn,'stat_global_team_hero',StatHerosTeam.Properties.VariableNames,StatHerosTeam,'id','public');
end
disp('Traitement OK')
end