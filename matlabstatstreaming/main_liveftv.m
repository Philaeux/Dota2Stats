...
while nolimit==0
    try
        disp(['start main_live_ftv loop time : ',datestr(datetime('now'))])
        tic
        %% con sql
        %% Set preferences
        ...
        load('sql.mat','config');
        connftv = database(config.mysql,config.mysqluser,config.mysqlpass);
        serveur_mysql_ok = -1;
        while serveur_mysql_ok ~= 0
            disp('Tentative de connexion avec MYSQL...');
            if isempty(connftv.Catalogs)==1
                disp('Echec de connexion à MYSQL, nouvelle tentative dans 1min...');
                pause(60)
                conn=database(config.mysql,config.mysqluser,config.mysqlpass);
            else
                serveur_mysql_ok = 0;
        ...
        %% update info player et team site ftv
        %updateinfoftv(conn,connftv);
        ...
        for i=1:height(tn_running)
            ...
            updateteam(conn,connftv,TktMatch)
            ...
            if strcmp(newmatchvalve,'No Data')==0
                for j=1:height(newmatchvalve)
                    ...
                    datavalve=guardian_match(conn,connftv,datavalve);
            ...
            if strcmp(newmatchopen,'No Data')==0
                for j=1:height(newmatchopen)
                    ...
                    if execmatchopenadd.execopenplayer==6
                        execguardian(dataod,conn);
                    end
