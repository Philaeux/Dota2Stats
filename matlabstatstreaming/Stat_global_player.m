...
if nb_player>0
    ...
    rqexist='select id from public.stat_global_player';
    exist1=pgsqldata(conn,rqexist);
    if strcmp(exist1,'No Data')==0
        pgsqlexec(conn,'delete from public.stat_global_player')
    end
    CBM_PGSQL_Transact_light(conn,'stat_global_player',StatPlayer.Properties.VariableNames,StatPlayer,'id','public');
end
