function [LIVE_TI7_MetaRecord,LIVE_TI7_PlayerRecords]=Twitterecord(BD_TI7_StatMatchPlayer,LIVE_TI7_PlayerRecords,LIVE_TI7_MetaRecord,LIVE_TI7_Team,LIVE_TI7_Player,LIVE_TI7_Hero,twfr,twen)
...
for i=1:10
    ...
    if BD_TI7_StatMatchPlayer.assists(i,1)>mean(LIVE_TI7_MetaRecord.value(strcmp(LIVE_TI7_MetaRecord.type,'maxassists')==1))
        ...
        try
            if length(msgfr)<141
                %twfr.updateStatus(msgfr,'media_ids',twti7.img.worldrec);
                twfr.updateStatus(msgfr);
            else
    ...
    elseif BD_TI7_StatMatchPlayer.assists(i,1)>LIVE_TI7_PlayerRecords.maxassists(LIVE_TI7_PlayerRecords.account_id==BD_TI7_StatMatchPlayer.account_id(i,1))
        ...
        try
            if length(msgfr)<141
                %twfr.updateStatus(msgfr,'media_ids',twti7.img.persorec);
                twfr.updateStatus(msgfr);
            else
                disp(['msg trop long : ',msgfr]);
            end
