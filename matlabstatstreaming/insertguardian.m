function execmatchadd=insertguardian(dataod,conn,execmatch)
...
try
    ...
    players=table();
    try
        disp(['Traitement api.open pour guardian de ',dataod.radiant_team.name,' vs ',dataod.dire_team.name])
    catch
    end
    execmatchadd=table();
    ...
    if isempty(dataod.teamfights)==0
        ...
        if execmatchadd.execopenpicks<5
            ...
            picks.game_mode=dataod.game_mode;
            ...
            %% insertion sql
            %% verifier si le match exist
            RQexist=['select id from public.guardian_openmatch where guardian_openmatch.match_id=',num2str(dataod.match_id)];
            existsql=pgsqldata(conn,RQexist);
            if strcmp(existsql,'No Data')==1
                CBM_PGSQL_inject1l_light(conn,'guardian_openmatch',picks.Properties.VariableNames,picks,'id',NaN,'public','add');
            else
                CBM_PGSQL_inject1l_light(conn,'guardian_openmatch',picks.Properties.VariableNames,picks,'id',existsql.id,'public','update');
            end
