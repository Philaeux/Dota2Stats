function Stat_global_hero(conn)
disp('Traitement des stats Global Heros')
...
if nb_match>0
    ...
    rqexist='select id from public.stat_global_hero';
    exist1=pgsqldata(conn,rqexist);
    if strcmp(exist1,'No Data')==0
        pgsqlexec(conn,'delete from public.stat_global_hero')
    end
    CBM_PGSQL_Transact_light(conn,'stat_global_hero',StatHero.Properties.VariableNames,StatHero,'id','public');
end
disp('Traitement OK')
end